    ...
    if (plannerState == 0) {
        ...
        while (start_set.size() > 0 && open_pq.size() > 0) {
            ...
            if (start_set.find(topPriorityNode->hash()) != start_set.end()) {
                ...
                cout << "target t: " << start_set[topPriorityNode->hash()] << ", node t: " << topPriorityNode->t << endl;
                if (start_set[topPriorityNode->hash()] > topPriorityNode->t) {
                    cout << "goal reachable" << endl;

                    int waitingTime = start_set[topPriorityNode->hash()] - topPriorityNode->t;
                    int pathCost = waitingTime * map[GETMAPINDEX(topPriorityNode->x,topPriorityNode->y,x_size,y_size)] + topPriorityNode->g;

                    candidate_set[pathCost] = topPriorityNode->hash();
                } else {
                    cout << "goal xreachable" << endl;
                }
