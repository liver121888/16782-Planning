    ...
    if (plannerState == 0) {
        ...
        while (start_set.size() > 0 && open_pq.size() > 0) {
            ...
            if (start_set.find(topPriorityNode->hash()) != start_set.end()) {
                ...
                cout << "target t: " << start_set[topPriorityNode->hash()] << ", node g: " << topPriorityNode->g << endl;
                if (start_set[topPriorityNode->hash()] > topPriorityNode->g) {
                    cout << "goal reachable" << endl;
                    // waiting time as key, g, hash as val
                    candidate_set[start_set[topPriorityNode->hash()] - topPriorityNode->g] = make_pair(topPriorityNode->g, topPriorityNode->hash());
                } else {
