...
// heuristics, hash
priority_queue<Node*> open_pq;

unordered_set<string> closed_set;

unordered_set<Node*> outdated_set;
...
void planner(
    ...
    if (plannerState == 0) {
        ...
        Node* dummy = new Node(-1, -1, 0, 0, 0, "", "");
        grid[dummy->hash()] = dummy;
        Node* start_state = new Node(robotposeX, robotposeY, 0, 0, 0, dummy->hash(), "");
